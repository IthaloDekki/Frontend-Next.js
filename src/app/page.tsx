...
function AddTask({ onAddTask }: { onAddTask: (taskText: string) => void }) {
  ...
  return (
    <div className="w-12/12 flex space-x-4 text-xl">
      ...
      <button onClick={handleAdd} className="bg-red-400  text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ml-2" >
        Enter
      </button>
    </div>
  );
...
function ProgressBar({ percentage }: { percentage: number }) {
  console.log("entrei progressbar");
  return (
    <>
      <div className="w-full mt-2 bg-red-800 rounded-full h-1.5 mb-4 dark:bg-red-200">
        <div
          style={{ width: `${percentage}%` }}
          className={`bg-red-400 h-1.5 rounded-full dark:bg-red-500`}
        ></div>
      </div>
    </>
  );
...
function AddedTask({ task, OnStatusChange }:  { OnStatusChange: (taskId: number) => void; task: Task; }) {
  console.log("entrei Added tasks");
  return (
    <div className="w-12/12 flex items-center space-x-2">
      <input onChange={(e) => OnStatusChange(task.id)} type="checkbox" checked={task.checked} className="w-6 h-6 text-red-400 bg-gray-100 border-red-300 rounded focus:ring-red-500 dark:focus:ring-red-800 dark:ring-offset-red-800 focus:ring-2 dark:bg-red-400 dark:border-red-400" />
      <label className="text-xl font-medium text-red-400">{task.text}</label>
    </div>
  );
}

function ListTasks({ tasks, OnStatusChange }: { OnStatusChange: (taskId: number) => void;  tasks: Task[]; }) {
  console.log("entrei listtasks");
...
function OptionsLine({ tasks,filter, OnFilterChange,OnDeleteDone}: { tasks: Task[]; filter: string; OnFilterChange: (filter: string) => void; OnDeleteDone: () => void; }) {
  return (
    <div className="w-12/12 flex items-center justify-center space-x-12">
      ...
      <button onClick={() => OnFilterChange("all")} className={`${ filter === "all" ? "border-b-2 border-red-400 text-red-200" : "" }`} >
        All
      </button>
      <button onClick={() => OnFilterChange("pending")} className={`${ filter === "pending" ? "border-b-2 border-red-400 text-red-200" : "" }`} >
        Pending
      </button>
      <button onClick={() => OnFilterChange("done")} className={`${ filter === "done" ? "border-b-2 border-red-400 text-red-200" : "" }`} >
        Done
      </button>
      <button onClick={() => OnDeleteDone()} className="px-4 cursor-pointer border-2 border-red-400 rounded text-red-200" >
        Clean Completed
      </button>
    </div>
  );
